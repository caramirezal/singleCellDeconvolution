...
```{r loading}
## paths
mtxDir <- "../data/chenS2019sameCellsData/GSE126074_P0_BrainCortex_SNAREseq_chromatin.counts.mtx"
barcodesDir <- "../data/chenS2019sameCellsData/GSE126074_P0_BrainCortex_SNAREseq_chromatin.barcodes.tsv"
peaksDir <- "../data/chenS2019sameCellsData/GSE126074_P0_BrainCortex_SNAREseq_chromatin.peaks.tsv"

## Loading data
Ad.chromatin.data=readMM(mtxDir)
colnames(Ad.chromatin.data)=as.character(unlist(read.table(barcodesDir)))
rownames(Ad.chromatin.data)=as.character(unlist(read.table(peaksDir)))
```
...
Ad.metadata.df <- rename(Ad.metadata.df, cluster=Ident)
tsne <- merge(Ad.chromatin.tsne, Ad.metadata.df)
...
tsne %>%
        #filter(Ident != 'Mis') %>%
           ggplot(aes(x=tSNE1, y=tSNE2, color=IdentChar
                      )) +
...
umap.df <- merge(umap.df, Ad.metadata.df)
umap.df %>%
        #filter(Ident != 'Mis') %>%
        ggplot(aes(x=UMAP1, y=UMAP2, color=IdentChar)) +
