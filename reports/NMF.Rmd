...
```{r atac_cistopic, eval=FALSE, echo=FALSE}
...
```{r nmfsatsne_unaligned, fig.width=10, echo=FALSE}
...
cell_types <- unique(cell_type$Ident)
shared <- matrix(0, length(cell_type$Ident), length(cell_types))
colnames(shared) <- cell_types
for (i in 1:nrow(shared)){
        cell_type_index <- which(cell_type$Ident[i]==cell_types) 
